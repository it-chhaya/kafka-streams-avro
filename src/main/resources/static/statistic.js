...
function connect() {
    ...
    stompClient.connect({},
        function(frame) {
            ...
            stompClient.subscribe('/topic/report1-request', function(payload) {
                console.log(payload)
                console.log('connected')
                onReport1Downloaded(payload.body)
            });
        },
        function(error) {
...
function onReport1Downloaded() {
    btnExcelReport1.innerHTML = 'Excel'
    localStorage.setItem('report1-processing', 'false')
}
...
window.onload = function () {
    ...
    let report1Processing = localStorage.getItem('report1-processing')
    if (report1Processing === 'true') {
        console.log('there is processing thread with report 1')
...
btnExcelReport1.onclick = function () {
    ...
    fetch('/api/v1/reports/download/report-1')
        ...
        .then(json => {
            ...
            `
            localStorage.setItem('report1-processing', 'true')
        })
}
