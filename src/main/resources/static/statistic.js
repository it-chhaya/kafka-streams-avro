...
function connect() {
    ...
    stompClient.connect({}, function (frame) {
            ...
            stompClient.subscribe('/topic/report1-download', function (downloadJob) {
                const data = JSON.parse(downloadJob.body);
                console.log(data)
                // Update UI (remove spinner)
                btnExcelReport1.innerHTML = `<span>Export</span>`
                localStorage.setItem('report1-downloading', 'false')
                btnExcelReport1.removeClass('cursor-not-allowed')
            });
        },
        function (error) {
            console.log('Error')
...
window.onload = function () {
    ...
    let report1Downloading = localStorage.getItem('report1-downloading')

    if (report1Downloading === 'true') {
        btnExcelReport1.innerHTML += `
                    <div role="status">
    ...
    fetch('/api/v1/reports/download/report-1')
        ...
        .then(json => {
            ...
            btnExcelReport1.innerHTML += `
                ...
                `
            btnExcelReport1.addClass('cursor-not-allowed')
            localStorage.setItem('report1-downloading', 'true')
        })
}
